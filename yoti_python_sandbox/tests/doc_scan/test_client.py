...
def test_configure_session_response_should_raise_sandbox_exception(_, doc_scan_sandbox_client):
    ...
    with pytest.raises(SandboxException) as ex:
        doc_scan_sandbox_client.configure_session_response(SOME_SESSION_ID, response_config_mock)
...
def test_configure_application_response_should_raise_sandbox_exception(_, doc_scan_sandbox_client):
    response_config_mock = mock.Mock(spec=ResponseConfig)
    response_config_mock.to_json.return_value = {"some": "object"}
